<script>
	import MailIcon from '~icons/ic/baseline-mail';
	import PhoneIcon from '~icons/ic/baseline-local-phone';
	import FacebookIcon from '~icons/ic/round-facebook';
	import InstagramIcon from '~icons/ri/instagram-fill';
</script>

<section id="ContactInformation" class="py-8 bg-secondary">
	<div class="mx-auto max-w-[30rem] lg:max-w-[100rem] px-4 sm:px-12">
		<h1 class="font-bold text-accent text-3xl/tight lg:text-4xl/tight xl:text-5xl/tight lg:w-3/4">
			Contact Information
		</h1>
		<p class="lg:w-3/4 font-medium text-base xl:text-lg text-white my-2">
			You can reach us through one of the following channels.
		</p>
		<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 my-8">
			<!-- email  -->
			<div
				class="flex flex-row items-center gap-4 px-4 py-5 bg-base-100 rounded-2xl text-secondary-highlight"
			>
				<MailIcon class="w-11 xl:w-12 h-11 xl:h-12 p-3 rounded-2xl bg-base-300 shadow-md" />
				<div class="">
					<p class="font-semibold text-lg lg:text-xl">Email</p>
					<a href="mailto:info@raglobal.tn" class="py-1 text-base lg:text-lg">info@raglobal.tn</a>
				</div>
			</div>
			<div
				class="flex flex-row items-center gap-4 px-4 py-5 bg-base-100 rounded-2xl text-secondary-highlight"
			>
				<PhoneIcon class="w-11 xl:w-12 h-11 xl:h-12 p-3 rounded-2xl bg-base-300 shadow-md" />
				<div class="">
					<p class="font-semibold text-lg lg:text-xl">Whatsapp</p>
					<a href="https://wa.me/+21648063955" class="py-1 text-base lg:text-lg">+216-48 063 955</a>
				</div>
			</div>
			<div
				class="flex flex-row items-center gap-4 px-4 py-5 bg-base-100 rounded-2xl text-secondary-highlight"
			>
				<FacebookIcon class="w-11 xl:w-12 h-11 xl:h-12 p-3 rounded-2xl bg-base-300 shadow-md" />
				<div class="">
					<p class="font-semibold text-lg lg:text-xl">Facebook</p>
					<a href="https://www.facebook.com/raglobal.tn" class="py-1 text-base lg:text-lg"
						>RA Global</a
					>
				</div>
			</div>
			<div
				class="flex flex-row items-center gap-4 px-4 py-5 bg-base-100 rounded-2xl text-secondary-highlight"
			>
				<InstagramIcon class="w-11 xl:w-12 h-11 xl:h-12 p-3 rounded-2xl bg-base-300 shadow-md" />
				<div class="">
					<p class="font-semibold text-lg lg:text-xl">Instagram</p>
					<a href="https://www.instagram.com/raglobal.tn/" class="py-1 text-base lg:text-lg"
						>raglobal.tn</a
					>
				</div>
			</div>
		</div>
		<!-- EMBED  -->
		<div class=" border-base-300 bottom-2 rounded-lg">
			<iframe
				src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d798.4591538080762!2d10.19205742849442!3d36.82243489825783!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x8b07d0809ea2bb57%3A0xc30ef22272097b22!2sRA%20Global!5e0!3m2!1sen!2stn!4v1724321889723!5m2!1sen!2stn"
				title="Location"
				height="550"
				style="border: 3px solid #FFF6EB;  border-radius: 16px; width: 100%"
				allowfullscreen=""
				loading="lazy"
				referrerpolicy="no-referrer-when-downgrade"
			></iframe>
		</div>
	</div>
</section>

<style></style>
